...
export function CitizenPortal({ projects, language, verifyId }: { projects: Project[]; language: PortalLanguage; verifyId?: string }) {
  ...
  return <main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <Panel className="overflow-hidden p-0"><div className="flex flex-wrap items-center gap-3 border-b border-[#334155]/70 p-4"><div className="mr-auto"><h2 className="text-sm font-black text-white">{t.nearby}</h2><p className="text-[11px] text-slate-400">{located ? 'Centered on your browser location' : 'Live spatial asset discovery powered by Supabase GIS'}</p></div><div className="relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search constituency or work" className="w-56 rounded-lg border border-[#334155] bg-[#0f172a] py-2 pl-8 pr-3 text-xs text-white outline-none" /></div></div><div className="relative"><div className="absolute left-4 top-4 z-[1000] rounded-lg border border-cyan-400/20 bg-[#0f172a]/90 px-3 py-2 text-[10px] font-bold text-cyan-200"><MapPin size={12} className="mr-1 inline" /> {nearby.length} mapped works</div><AssetMap assets={nearby} onSelect={setSelected} /><div className="absolute bottom-4 left-4 z-[1000] flex gap-4 rounded-lg border border-[#334155] bg-[#0f172a]/90 px-3 py-2 text-[10px] text-slate-300"><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400" />{t.completed}</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-amber-400" />{t.inProgress}</span></div></div></Panel>
